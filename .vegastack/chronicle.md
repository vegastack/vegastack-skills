...
- **What:** Six small rendering annoyances are gone. A project's review known-patterns file now shows one line per field instead of a merged paragraph (the same bug [#44](https://github.com/vegastack/vegastack-skills/issues/44) fixed for the chronicle, in the template consuming repos inherit); changesets now have a stated shape, so future changelog entries and the release notes that lead with them stop being 1,200-character walls; the evidence comment's sha is a clickable commit link; the terminal status board no longer prints raw link markup in the last chronicle title; the README skills table reads as one line per skill; and two archived plan documents lost their merged headers.
...
- **How it went:** The edits themselves were straightforward, and the sweep found a seventh case the brief had not named (a second archived plan with the same merged header), folded in rather than left to make the acceptance claim false. Review was the harder part: it caught the build's own bookkeeping — no task checkpoints in the ledger and every plan checkbox unticked, which would have made a resumed session redo finished work — plus a rule about scannable changelogs delivered in the least scannable sentence in the skill, and three copies of that rule where one home was the point. Fifteen findings, all fixed on the record.
- **Changed:** Known-patterns template bulleted · changeset shape convention (dev-implement's rule, cited by CONTRIBUTING and release-ops) · linked sha in the evidence tail, guard widened · `titlePlain`/`gistPlain` from status.mjs · one-line README rows · legacy plan headers.
- **Decisions:** none.
