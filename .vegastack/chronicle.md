...
- **How it went:** the numbers decided most of the design. Measuring first showed the aggregate risk score is unusable here as a gate — `skill-maintainer` scored 80 HIGH entirely because its SKILL.md cites repo-root paths that cannot resolve inside a skill directory, so a score gate would have blocked every push forever on a non-finding. The same measurements killed two other tempting choices: scan the built bundle rather than the authored tree, because unpackaged `tests/` fixtures are deliberately adversarial and score higher than anything that ships; and keep the LLM pass out of the guard, because a recursive semantic run omitted eleven of twelve skills on an internal safety limit and reported a *degraded* skill at 98 CRITICAL. Two defects were caught by the tests rather than by review: the scanner subprocess inherited no environment under Bun, silently dropping the test seam and any scanner configuration — `ship-gate.mjs` already carried the fix and it had been overlooked; and enforcing the suppression discipline only on `rules:` would have been bypassed entirely by committing the output of `skillspector baseline`, which writes every finding as a fingerprint with a default reason.
- **Changed:** a new `skills/dev-skills/dev-review/scripts/skill-scan.mjs` with pure, unit-driven baseline parsing, skill discovery, and verdict functions plus a fake-scanner fixture · the `skill-scan:` knob in the profile template and dev-setup's detection · the Verify-gate bullet in `dev-implement` · a "Scanner evidence" method in the Security axis and its dispatch brief · the suppression discipline in `skill-maintainer` · this repo's knob, seed baseline, Verify and Ship lines, and contributor install instructions.
- **Decisions:** skill scanning is a first-class workflow stage — a deterministic guard at Verify blocking on unsuppressed HIGH/CRITICAL, triaged by dev-review's Security axis, with suppressions as a justified, reviewed baseline.
