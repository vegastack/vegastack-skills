...
- auto: when `packages/broker/action/` changed since the `factory-token-v1` tag (`git diff --quiet factory-token-v1 -- packages/broker/action || echo moved`), move that tag to the same release commit — `git tag -f factory-token-v1 && git push --force origin refs/tags/factory-token-v1` — covered by the release word; the ship guard still prompts on the `--force`, which is the one force-push the runbook expects (the action ships from this repository by path, decision 04-09-2026)
