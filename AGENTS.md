...
- **Layout is exactly two levels:** `skills/<group>/<name>/`, never deeper, with a `GROUP.md` per group — currently `dev` and `repo-tooling`. The published bundle is flat, so skill names are unique across the whole tree and install commands never carry a group.
