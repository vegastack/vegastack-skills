...
describe('publishable package', () => {
  test('packs locally, installs the tarball, and runs its bundled installer', async () => {
    ...
    const compliant = resolve(packageRoot, '../../skills/vegastack-arch-guardian/tests/fixtures/compliant')
    await cp(compliant, project, { recursive: true })
    const add = Bun.spawnSync([cli, 'add', 'vegastack-arch-guardian', '--agent', 'both', '--dir', project, '--non-interactive'], { cwd: temporary, env: { ...process.env, HOME: temporary } })
    expect(add.exitCode).toBe(0)
    expect(await readFile(join(project, '.agents/skills/vegastack-arch-guardian/SKILL.md'), 'utf8')).toContain('name: vegastack-arch-guardian')
    expect(await readFile(join(project, '.claude/skills/vegastack-arch-guardian/SKILL.md'), 'utf8')).toContain('name: vegastack-arch-guardian')
    const installed = join(project, '.agents/skills/vegastack-arch-guardian')
    ...
    for (const required of ['references/workflows.md', 'references/profile-governance.md', 'references/golden-patterns.md', 'references/foundation-compatibility.json', 'references/control-catalog.json', 'references/rule-model.json', 'scripts/profile-tool.mjs', 'scripts/schema-validate.mjs']) expect(installedFiles.some(path => path.endsWith(required))).toBe(true)
    ...
    const architecture = Bun.spawnSync(['node', join(installed, 'scripts/architecture-check.mjs'), project], { cwd: project })
