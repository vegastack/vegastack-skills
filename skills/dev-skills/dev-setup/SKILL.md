...
| agent skills in the repo | a directory whose immediate children hold `SKILL.md` files (or that holds one itself) — drafts the `skill-scan:` knob at that path; none found drafts `none`. Where the project builds a flattened bundle, the knob names the **built** directory: unpackaged `tests/` fixtures are deliberately adversarial and score higher than what ships |
