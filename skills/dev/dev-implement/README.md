...
| [references/parallel-children.md](references/parallel-children.md) | Running a parent's independent children at the same time: the `children.mjs` verbs, the Claude workflow and Codex `codex exec -C` paths, the fallback ladder, the concurrency caps, the fast-forward join with its declared-file-set scope check, and the ledger and removal rules |
