...
- Workflows that write issues, labels, or project items → offer the App-token recipe in [github-app](references/github-app.md) rather than a personal access token: a PAT carries its owner's whole account, the App a named permission set that uninstalling revokes at once. The App, its key and the org secret stay the operator's own actions, drafted here and never performed
