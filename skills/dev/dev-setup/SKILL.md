...
| guard workflows / hook files | only the ones the user said yes to in Round C; hook files are copied verbatim from `assets/hooks/` to `.vegastack/hooks/`, and the wiring goes to `.claude/settings.json` (Claude Code), `<repo>/.codex/hooks.json` (Codex) and `~/.hermes/config.yaml` (Hermes, ship guard only), merged never replaced |
