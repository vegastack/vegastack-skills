...
- Statistics capture (the org's `stats:` policy is on, or no control room exists yet) → offer the three capture hooks separately, one plain sentence each: a record per closed session; which skills a Claude session used and who chose each; the same on Codex from `$name` mentions, a proxy recorded as one. Say what a record holds — counts and identifiers, never text — that it goes to the org's own control room over the operator's `gh` credential, and that whether anything is recorded is the org's `stats:` policy, not this machine's
