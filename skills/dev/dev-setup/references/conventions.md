...
`rev=<n>` and the matching `(v<n>)` heading suffix appear only on revisable artifacts — the brief (issue description), `plan`, `evidence` — starting at `rev=1`/`(v1)`. Single-event comments (`approval`, `decision`, `handback`) and the `ledger` carry neither. Scripts and agents locate comments by marker, never by heading text; a comment without its marker is not the artifact — no legacy fallback.

## Operator identity

Every human reference in every artifact — approvals, revisions, decisions, changelog attributions, review adjudications — names the operator by GitHub username in parentheses, no title — `(<github-username>)`:
...
- **`quick-build`** — small change and the flow being changed already exists in the repo to read. Brief and plan are drafted in the same conversation; **one approval covers both**; then straight to `ready`.
...
State — exactly one per issue, and every flip sets the assignee column (creation colors live in dev-setup's labels row, their one home):
...
Modifiers (may coexist with the state label): `risky` · scope `research` / `quick-build` / `full-plan` · `epic` (map parents, where the org has no native Epic issue type).
