...
- **Telemetry.** Claude Code exports OpenTelemetry metrics and log events; `claude_code.skill_activated` is "logged when a skill is invoked, whether Claude calls it through the Skill tool or you run it as a `/` command", carrying `invocation_trigger` (`"user-slash"`, `"claude-proactive"` or `"nested-skill"`) and `skill.source`. `skill.name` is the placeholder `"custom_skill"` for user-defined and third-party plugin skills unless `OTEL_LOG_TOOL_DETAILS=1`. Verified 03-09-2026. <!-- source: CC-TELEMETRY -->
- `OTEL_LOG_TOOL_DETAILS=1` "enable[s] logging of tool parameters and input arguments in tool events and trace span attributes: Bash commands, MCP server and tool names, skill names, user-authored workflow names, and tool input" — it is off by default, and this workflow never turns it on: a statistics record carries counts and identifiers, never a tool argument. Verified 03-09-2026. <!-- source: CC-TELEMETRY -->
...
- **Telemetry.** OTel log export is **disabled by default** and opted into with an `[otel]` table in config.toml (`exporter = "none"`, `otlp-http` or `otlp-grpc`); with `"none"` Codex "records events but sends nothing". The documented stream covers "API requests, SSE/events, prompts, tool approvals/results" and names no skill-activation event — which is why skill capture on Codex is a `$name` prompt proxy and is recorded as one. Verified 03-09-2026. <!-- source: CODEX-OTEL -->
...
- The **Curator** is "a background maintenance pass for agent-created skills": it tracks how often each skill is viewed, used and patched, moves long-unused skills through active → stale → archived, and is triggered by an inactivity check rather than a cron daemon (`interval_hours` default 7 days, `min_idle_hours` default 2 hours). It never auto-deletes — the worst outcome is archival into `~/.hermes/skills/.archive/`. It is a maintenance pass over skills, not a usage-statistics stream, so it answers nothing this workflow's records answer. Verified 03-09-2026. <!-- source: HERMES-CURATOR -->
...
- The OpenTelemetry stream is **optional and never required**. Statistics capture is deterministic without it: the dispatcher parses each harness's own machine-readable run output, SessionEnd hooks cover interactive sessions, and skill invocations come from hook payloads. An org that wants the richer stream can turn OTel on — but nothing in this workflow depends on a collector existing, and `OTEL_LOG_TOOL_DETAILS` stays off because it exports exactly the tool arguments a record must never hold.
