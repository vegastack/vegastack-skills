...
2. Maintainer, at release time: `bunx changeset version` (applies changesets to `packages/cli/package.json` and the changelog), then `bun install` so any dependency changes riding along reach the lockfile, commit. That install does **not** update the workspace's own version inside `bun.lock`: with bun (the root `packageManager` line is the version of record), the recorded version is left unchanged by a plain install, by `--force`, and even by `--lockfile-only`; it catches up on a later change to a *declared* dependency range, whether or not that changes what resolves. `bun install --frozen-lockfile`, which CI and the release workflow both run, passes with the older recorded version, and nothing in this repo reads it: `packages/cli/package.json` is the single release identity. A `--frozen-lockfile` failure in a workspace never comes from that stale record — it comes from an internal `workspace:` range no longer matching the bumped package's manifest, which `changeset version` updates together and which this repo has no instance of. Never hand-edit the lockfile to "correct" it. The release record is `packages/cli/CHANGELOG.md`, changesets-written — never by hand; the root `CHANGELOG.md` is the frozen pre-0.3.0 record pointing there.
