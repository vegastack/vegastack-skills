...
2. Maintainer, at release time: `bunx changeset version` (applies changesets to `packages/cli/package.json` and the changelog), then `bun install` so any dependency changes riding along reach the lockfile, commit. That install does **not** update the workspace's own version inside `bun.lock`: with bun (the root `packageManager` line is the version of record), a version-only bump survives a plain install, `--force`, and even `--lockfile-only` untouched — the recorded version catches up the next time a *declared* dependency range changes, whether or not that changes what resolves. `bun install --frozen-lockfile`, which CI and the release workflow both run, passes with the older recorded version, and nothing reads it: `packages/cli/package.json` is the single release identity. (The one case where a stale record does break `--frozen-lockfile` is an internal `workspace:` dependency range, which this repo does not have.) Never hand-edit the lockfile to "correct" it. The release record is `packages/cli/CHANGELOG.md`, changesets-written — never by hand; the root `CHANGELOG.md` is the frozen pre-0.3.0 record pointing there.
