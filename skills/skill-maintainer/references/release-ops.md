...
2. Maintainer, at release time: `bunx changeset version` (applies changesets to `packages/cli/package.json` and the changelog), then `bun install` so any dependency changes riding along reach the lockfile, commit. That install does **not** re-pin the workspace's own version in `bun.lock` — bun rewrites the lockfile only when the dependency graph changes, `bun install --frozen-lockfile` passes with the older pin present, and nothing reads it; the manifest is the single release identity (verified 29-08-2026, bun 1.3.10). Never hand-edit the lockfile to "correct" it. The release record is `packages/cli/CHANGELOG.md`, changesets-written — never by hand; the root `CHANGELOG.md` is the frozen pre-0.3.0 record pointing there.
